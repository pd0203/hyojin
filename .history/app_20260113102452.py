...
def _calculate_monthly_salary(emp_id, year, month):
    ...
    for sunday in sundays_in_month:
        ...
        week_attendance = supabase.table('attendance_logs').select('clock_in, clock_out').eq('employee_id', emp_id).gte('work_date', week_start).lte('work_date', week_end).execute()
