...
def check_star_delivery(df):
    ...
    star_count = int(mask.sum())  # numpy int64 -> Python int
...
def filter_star_delivery(df):
    ...
    deleted_count = int(result['star_count'])  # Python int 보장
...
def get_margin_data():
    ...
    return jsonify({'data': MARGIN_DATA, 'total': len(MARGIN_DATA)})
...
def classify_orders():
    ...
    # 스타배송 필터링 옵션 확인
    filter_star = request.form.get('filter_star', 'false') == 'true'
    ...
    try:
        ...
        star_deleted_count = 0
        
        # 스타배송 필터링 옵션이 켜져 있을 때
        if filter_star:
            star_check = check_star_delivery(df)
            
            # 이미 필터링된 파일인지 확인 (스타배송 주문이 0건)
            if star_check['has_column'] and star_check['star_count'] == 0:
                return jsonify({
                    'error': 'already_filtered',
                    'message': '이미 스타배송 필터링이 완료된 파일입니다. 체크박스를 해제 후 다시 업로드해주세요.'
                }), 400
            
            # 스타배송 필터링 실행
            df, star_deleted_count = filter_star_delivery(df)
        ...
        if filter_star:
            stats['star_filter'] = {
                'enabled': True,
                'deleted_count': star_deleted_count
            }
